(function (global, factory) {
  ...
  function workLoop(hasTimeRemaining, initialTime) {
    ...
    while (currentTask !== null && !(enableSchedulerDebugging )) {
      if (currentTask.expirationTime > currentTime && (!hasTimeRemaining || shouldYieldToHost()) // 判断是不是过期
      ) {
      ...
      if (typeof callback === 'function') {
        ...
        var didUserCallbackTimeout = currentTask.expirationTime <= currentTime; // 回调是不是已经过期
        ...
        currentTime = getCurrentTime(); // 如果callback执行之后的返回类型是function类型就把又赋值给currentTask.callback，说明没执行完，这个逻辑在performConcurrentWorkOnRoot里面
  ...
  function unstable_scheduleCallback(priorityLevel, callback, options) {
    ...
    if (startTime > currentTime) {
      ...
      if (!isHostCallbackScheduled && !isPerformingWork) {
        isHostCallbackScheduled = true; // flushWork消费任务，会在浏览器的下一个事件循环执行。
